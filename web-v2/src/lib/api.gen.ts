...
export interface paths {
    ...
    "/v1/webhook-endpoints/endpoints": {
         ...
         * @description Returns the endpoint plus its signing secret (shown once). Event
        ...
        post: {
            ...
            responses: {
                /** @description Endpoint created (response carries the one-time signing secret) */
                201: {
                    headers: {
                        [name: string]: unknown;
                    };
    ...
    "/v1/webhook-endpoints/endpoints/{id}": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
         ...
         * @description Mutates url / description / events / active WITHOUT rotating the
    ...
    "/v1/webhook-endpoints/endpoints/{id}/rotate-secret": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
        ...
        post: {
            ...
            requestBody?: never;
            responses: {
                /** @description New secret (old secret keeps verifying during the grace window) */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
...
export interface components {
    schemas: {
        ...
        Invoice: {
            ...
            /** @description Human period string with the inclusive last covered day ("Jun 1, 2028 – Jun 30, 2028"), date-only in the invoice's billing timezone (ADR-058 / ADR-077). Computed on read; the raw billing_period_start/end stay half-open. Empty/omitted for one-off invoices with no period. */
            billing_period_display?: string;
            /** @description IANA timezone the period boundaries are anchored in — the org billing timezone resolved and denormalized onto the invoice at issue (ADR-077). billing_period_display is rendered in this zone, so an issued invoice's dates stay fixed even if the org later changes its timezone. Empty for ad-hoc/legacy invoices. */
            ...
            /**
             * @description Source Stripe invoice id (`in_xxx`) for an invoice imported from
