...
export default function DashboardPage() {
  ...
  const billingMutation = useMutation({
    ...
    onError: (err) => setBillingResult('Failed: ' + (err instanceof Error ? err.message : 'unknown')),
  })
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
            <Card>
              <CardContent className="p-5">
                ...
                <div className="flex items-baseline gap-2 mt-1">
                  <p className="text-[28px] font-semibold tabular-nums text-foreground">{formatCents(overview.mrr)}</p>
                  {chartData.length >= 2 && (() => {
                    ...
                    return <span className={cn('text-xs font-medium', pct >= 0 ? 'text-emerald-600' : 'text-destructive')}>{pct >= 0 ? '↑' : '↓'} {Math.abs(pct)}%</span>
                  })()}
                </div>
                {chartData.length > 1 && (
                  <div className="mt-2 h-[28px]">
                    <ResponsiveContainer width="100%" height={28}>
                      <AreaChart data={chartData.slice(-10)} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                        <defs><linearGradient id="mrrFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#635BFF" stopOpacity={0.15} /><stop offset="100%" stopColor="#635BFF" stopOpacity={0} /></linearGradient></defs>
            ...
            <Card>
              <CardContent className="p-5">
                ...
                <p className="text-[28px] font-semibold tabular-nums text-foreground mt-1">{overview.active_customers}</p>
