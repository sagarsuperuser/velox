...
// TestBillOnCycleReset_PaidInAdvance_GrantsCredit locks in the
// proration-credit path for the operator-driven Reset Billing Cycle
// flow. The user-visible regression that prompted this test: an
// in_advance sub with a PAID source invoice was reset mid-cycle and
// no credit grant fired — silent gap. The cause was a buggy "Start
// of next month" preset that mis-anchored the date; this test
// verifies the engine code path itself works end-to-end so any future
// regression in the engine is caught synchronously.
func TestBillOnCycleReset_PaidInAdvance_GrantsCredit(t *testing.T) {
	periodStart := time.Date(2029, 6, 21, 0, 0, 0, 0, time.UTC)
	periodEnd := time.Date(2029, 7, 21, 0, 0, 0, 0, time.UTC)
	anchorAt := time.Date(2029, 7, 1, 0, 0, 0, 0, time.UTC) // mid-period

	sub := domain.Subscription{
		ID: "sub_1", TenantID: "t1", CustomerID: "cus_1", Code: "advance",
		Status: domain.SubscriptionActive,
	...
	pricing := &mockPricing{plans: map[string]domain.Plan{
		"pln_advance": {
			ID: "pln_advance", Name: "Advance", Currency: "USD",
			BillingInterval: domain.BillingMonthly, BaseAmountCents: 5900,
			BaseBillTiming: domain.BillInAdvance,
		},
	}}
	paidLineStart := periodStart
	paid := domain.Invoice{
		ID: "inv_1", TenantID: "t1", SubscriptionID: "sub_1",
		Status: domain.InvoiceFinalized, PaymentStatus: domain.PaymentSucceeded,
	}
	paidLine := domain.InvoiceLineItem{
		ID: "ili_1", InvoiceID: "inv_1", LineType: domain.LineTypeBaseFee,
		BillingPeriodStart: &paidLineStart,
	}
	inv := &mockInvoices{invoices: []domain.Invoice{paid}, lineItems: []domain.InvoiceLineItem{paidLine}}
	granter := &fakeCreditGranter{}

	engine := wireBaseTax(NewEngine(&mockSubs{}, &mockUsage{}, pricing, inv, nil, &mockSettings{}, nil, nil, billingTestClock()))
	engine.SetCreditGranter(granter)

	cents, err := engine.BillOnCycleReset(context.Background(), sub, periodStart, periodEnd, anchorAt)
	if err != nil {
		t.Fatalf("BillOnCycleReset: %v", err)
	}
	if cents <= 0 {
		t.Errorf("expected proration credit > 0, got %d", cents)
	}
	if len(granter.grants) != 1 {
		t.Fatalf("expected 1 credit grant, got %d", len(granter.grants))
	}
	// Anchor at Jul 1 of 30-day period (Jun 21 → Jul 21): unused = 20 days.
	// 5900 * 20 / 30 = 3933 cents (round-half-to-even on int math).
	wantCents := int64(5900 * 20 / 30)
	if cents != wantCents {
		t.Errorf("credit cents: got %d, want %d (20-day unused of 30-day cycle)", cents, wantCents)
	}
	if granter.grants[0].SourceSubscriptionID != "sub_1" {
		t.Errorf("source_subscription_id: got %q, want sub_1", granter.grants[0].SourceSubscriptionID)
	}
}

// TestBillOnCycleReset_PaidInAdvance_UnpaidSource_NoCredit locks in
// the paid-check gate — matches the cancel-proration shape. An
// in_advance source invoice that wasn't paid means there's nothing
// to refund.
func TestBillOnCycleReset_PaidInAdvance_UnpaidSource_NoCredit(t *testing.T) {
	periodStart := time.Date(2029, 6, 21, 0, 0, 0, 0, time.UTC)
	periodEnd := time.Date(2029, 7, 21, 0, 0, 0, 0, time.UTC)
	anchorAt := time.Date(2029, 7, 1, 0, 0, 0, 0, time.UTC)
	...
	pricing := &mockPricing{plans: map[string]domain.Plan{
		"pln_advance": {ID: "pln_advance", BaseAmountCents: 5900, BaseBillTiming: domain.BillInAdvance, BillingInterval: domain.BillingMonthly},
	}}
	unpaidLine := periodStart
	inv := &mockInvoices{
		invoices: []domain.Invoice{{
			ID: "inv_1", TenantID: "t1", SubscriptionID: "sub_1",
			Status: domain.InvoiceFinalized, PaymentStatus: domain.PaymentPending,
		...
		lineItems: []domain.InvoiceLineItem{{
			InvoiceID: "inv_1", LineType: domain.LineTypeBaseFee, BillingPeriodStart: &unpaidLine,
		}},
	}
	granter := &fakeCreditGranter{}
	engine := wireBaseTax(NewEngine(&mockSubs{}, &mockUsage{}, pricing, inv, nil, &mockSettings{}, nil, nil, billingTestClock()))
	...
	if cents != 0 || len(granter.grants) != 0 {
		t.Errorf("unpaid source must suppress credit: got cents=%d, grants=%d", cents, len(granter.grants))
	}
}
