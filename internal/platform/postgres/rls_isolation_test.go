...
// TestRLSIsolation_FeatureFlagOverrides asserts that migration 0113 brought
// feature_flag_overrides — the last tenant-scoped table left without RLS —
// in line with every other tenant table: a tenant_isolation policy that
// blocks cross-tenant reads and writes under the velox_app role. The table is
// account-level config (no livemode column, per 0020), so the policy is
// tenant-only — it must NOT carry a livemode predicate.
func TestRLSIsolation_FeatureFlagOverrides(t *testing.T) {
	db := testutil.SetupTestDB(t)
	ctx, cancel := context.WithTimeout(postgres.WithLivemode(context.Background(), true), 10*time.Second)
	defer cancel()

	tenantA := testutil.CreateTestTenant(t, db, "FFO RLS A")
	tenantB := testutil.CreateTestTenant(t, db, "FFO RLS B")

	assertRLSEnabled(t, db, ctx, "feature_flag_overrides")

	// feature_flag_overrides has no livemode column — its policy must be
	// tenant-only. A livemode predicate here would be a copy-paste bug.
	tx, err := db.BeginTx(ctx, postgres.TxBypass, "")
	if err != nil {
		t.Fatalf("begin bypass: %v", err)
	}
	...
	if err := tx.QueryRowContext(ctx,
		`SELECT qual FROM pg_policies WHERE tablename = 'feature_flag_overrides' AND policyname = 'tenant_isolation'`,
	).Scan(&qual); err != nil {
		t.Fatalf("read feature_flag_overrides policy: %v", err)
	}
	if containsLivemodePredicate(qual) {
		t.Fatalf("feature_flag_overrides policy must be tenant-only (no livemode column); qual was: %s", qual)
	}
	postgres.Rollback(tx)

	seedFlagOverride(t, db, ctx, tenantA, "ffo_test_flag")
	seedFlagOverride(t, db, ctx, tenantB, "ffo_test_flag")

	// Acting as A, a no-filter scan sees exactly one row — A's, never B's.
	assertOnlyTenantVisible(t, db, ctx, tenantA,
		`SELECT tenant_id FROM feature_flag_overrides`, 1)

	// Write isolation: acting as A, inserting a row tagged with B's id fails.
	assertFlagOverrideInsertDenied(t, db, ctx, tenantA, tenantB, "ffo_test_flag")
}

func seedFlagOverride(t *testing.T, db *postgres.DB, ctx context.Context,
	tenantID, flagKey string) {
	t.Helper()
	...
	// feature_flags is global (no RLS) — the FK target must exist first.
	if _, err := tx.ExecContext(ctx,
		`INSERT INTO feature_flags (key, enabled) VALUES ($1, false)
		ON CONFLICT (key) DO NOTHING`, flagKey); err != nil {
		t.Fatalf("seed feature_flags: %v", err)
	}
	if _, err := tx.ExecContext(ctx,
		`INSERT INTO feature_flag_overrides (flag_key, tenant_id, enabled) VALUES ($1, $2, true)`,
		flagKey, tenantID); err != nil {
		t.Fatalf("seed feature_flag_overrides for %s: %v", tenantID, err)
	}
	if err := tx.Commit(); err != nil {
		t.Fatalf("seed commit: %v", err)
	}
...
func assertFlagOverrideInsertDenied(t *testing.T, db *postgres.DB, ctx context.Context,
	actingTenant, targetTenant, flagKey string) {
	t.Helper()
	...
	// Reuse the seeded flag key (FK target exists). The RLS WITH CHECK clause
	// rejects the cross-tenant tenant_id before any PK check is reached.
	_, err = tx.ExecContext(ctx,
		`INSERT INTO feature_flag_overrides (flag_key, tenant_id, enabled) VALUES ($1, $2, true)`,
		flagKey, targetTenant)
	if err == nil {
		t.Fatalf("RLS LEAK: tenant %s inserted a feature_flag_overrides row tagged with tenant %s",
			actingTenant, targetTenant)
	}
}
