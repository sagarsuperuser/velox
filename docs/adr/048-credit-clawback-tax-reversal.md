...
- **Open follow-up:** the **downgrade-clawback** leg shares the single-source resolution but claws back only the *removed* value (the delta), so a correct multi-invoice fan-out must attribute the clawback to the invoice(s) that funded the removed value — distinct from cancel's uniform unused-prepayment clawback. Deferred to its own design; it already fails loud today (no silent drop).
