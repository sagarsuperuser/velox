...
## Re-examination (2026-07-07): nominal rate + per-million — the effective rate is the non-standard choice; deferred

A sub-cent usage line surfaced `$0.00000333333333` (2¢ ÷ 6,000 tokens). It's *correct* (the effective rate at 12dp) but reads like a glitch — a repeating decimal only exists because we display the **effective** rate (`amount ÷ qty`), which doesn't terminate. Fresh research (verified quotes/URLs below) shows the effective rate is itself the **non-standard** choice:

- **Peers show the *nominal/configured* rate, not `amount ÷ qty`.** Stripe displays the rate you set — *"the quantity of MB is then multiplied by 0.05 cents and rounded to the nearest whole cent"* — rounding at the *line* level ([Stripe usage pricing](https://docs.stripe.com/billing/subscriptions/usage-based-legacy/pricing-models)). A configured rate (`0.0003`) terminates, so no repeating decimal ever appears. The peers sidestep our problem by showing nominal, not effective.
- **AI-native peers show *per-million* for tokens.** Metronome's quantity-conversion lets you *"send individual tokens but **display and price per million tokens on the invoice**"* ([Metronome](https://docs.metronome.com/guides/get-started/metronome-dashboard-quickstart)); frontier pricing (Anthropic, OpenAI) is universally quoted `$/1M tokens`. The real token convention is **"$3.00 / 1M tokens"**, not any per-token rate.
- **Exact reconciliation is NOT an industry requirement** — which weakens this ADR's *main* reason (§Consequences, §Risks) for preferring effective over nominal. Systems routinely display `qty × unit ≠ rounded amount`: ProjectWorks computes qty to 6dp but *displays* 4dp; ERP.net rounds the line to 2dp; the EU e-invoice standard rounds intermediates ([ProjectWorks](https://help.projectworks.com/knowledge/showing-qty-x-rate-on-invoices), [ERP.net](https://docs.erp.net/tech/modules/crm/invoicing/determine-unit-price-and-line-amount-in-invoice-orders.html)).

**Decision: keep the effective-rate display for now; do NOT band-aid it (no sig-fig trim — that isn't what anyone does).** Rationale: the money is correct, we are pre-launch with zero customers reading invoices, and the proper fix is a money-path change reversing this ADR — no named pressure justifies it yet. The effective rate stays correct and remains the right answer for tiered/blended/multi-dimensional lines that have no single nominal rate.

**Agreed future direction (when triggered):** show the **nominal** rate for flat single-rule lines — derivable on read from the immutable `rating_rule_versions` row the line already references via `RatingRuleVersionID` (no migration, no drift; ADR-054's derive-on-read posture preserved), falling back to the effective rate for non-flat lines — and add a **per-million display scale** for token meters. **Trigger:** first design partner reviewing invoices (or a sub-cent-rate display complaint). See memory `project_unit_rate_display_nominal_vs_effective`.
