...
| **`SettleFailed` event/email/dunning in-tx + dunning recovery** — `payment.failed`, the failed-email, AND `StartDunning` all fire post-commit, all gated behind `firstForThisPI` AFTER `MarkPaymentFailedReportingTransition` commits `failure_notified_pi`. So a crash in that window loses all three, and a same-PI redelivery returns `firstForThisPI=false` → skips them; the reconciler only sweeps `unknown`/`processing`, never `failed`, so it doesn't recover them. ⚠️ The UNIQUE-per-invoice constraint (0085) protects against *double*-start, NOT *never*-start — dunning genuinely doesn't auto-start in this window (only operator-visible via the attention banner). Two-part fix: (a) move `payment.failed` + the email **in-tx** (symmetric to the success-path fix); (b) `StartDunning` can't go in-tx (cross-domain + sleeps ~600ms) → add a **reconciler sweep for `failed` invoices with no dunning run** (idempotent start). | settlement.go (no ADR) | `internal/payment/settlement.go` · `SettleFailed` (StartDunning ~:246) | next time this path is touched, or guaranteed failure-notification + collection delivery |
| **`amount_paid` edge — Part B: automated clawback defer** — the operator vector is now gated (Part A, shipped 2026-06-22). But the AUTOMATED ADR-050 unpaid-source clawback (`CreateAndIssueAdjustment`) still reduces `amount_due` on a `processing` source between PI-create and settle → `MarkPaid` records `amount_paid` too low → refund cap under-sized. Fix = defer that reduction until the payment settles (can't 409 it — the cancel/downgrade must proceed). | MarkPaid `KNOWN EDGE` comment (no ADR) | `internal/billing/engine.go` · `relieveUnpaidPrebill` / `internal/creditnote/service.go` · `CreateAndIssueAdjustment` | a first async / EU-SCA payment-method design partner |
| **`amount_paid` edge — Part C: record from captured** — `MarkPaid` records `amount_paid = amount_due` at settle. Under Velox's full-capture model (+ Part A's gate) that equals the captured amount; under **partial capture** it wouldn't. Record from the PI's `amount_received` (the processor's captured amount) instead — the clean source-of-truth. | MarkPaid `KNOWN EDGE` comment (no ADR) | `internal/invoice/postgres.go` · `MarkPaid` | partial-capture support |
