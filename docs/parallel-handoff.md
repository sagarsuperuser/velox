...
## 2026-04-26 (Sun) — Track A: plan migration tool (Week 6)
...
- **Shipped (Week 6 deliverable #2):**
  ...
  - Router wiring in `internal/api/router.go` — `planMigrationStore` + `planMigrationSvc` + `planMigrationH` constructed below `createPreviewH`, mounted at `/admin/plan_migrations` gated by `PermSubscriptionWrite` (both preview and commit are write-grade — cohort selection can be sensitive even when no DB mutation occurs).
  - `internal/planmigration/wire_shape_test.go` — three regression tests pin the snake_case contract: `TestWireShape_PlanMigrationPreview_SnakeCase`, `TestWireShape_PlanMigrationCommit_SnakeCase`, `TestWireShape_PlanMigrationList_SnakeCase`. Each asserts the documented snake_case keys, the absence of PascalCase leakage, always-array idioms (`warnings`, `totals`), always-object `customer_filter`, JSON-number (not string) numeric fields.
  - `web-v2/src/lib/api.ts` — `previewPlanMigration` / `commitPlanMigration` / `listPlanMigrations` plus the seven type interfaces (`PlanMigrationCustomerFilter`, `PlanMigrationPreviewRequest`, `PlanMigrationCustomerPreview`, `PlanMigrationTotal`, `PlanMigrationPreviewResponse`, `PlanMigrationCommitRequest`, `PlanMigrationCommitResponse`, `PlanMigrationListItem`, `PlanMigrationListResponse`).
  - `web-v2/src/pages/PlanMigration.tsx` — three-step flow (configure → preview → commit). Configure picks `from_plan` / `to_plan` / `customer_filter` (`all` / `ids` / `tag`-disabled) / `effective`. Preview renders the cohort table with per-row delta highlighting + warnings panel + cohort total card. Commit modal auto-generates an idempotency key via `crypto.randomUUID()` (editable for retry scenarios) and guards against zero-cohort commits. Recent-migrations sidebar surfaces the last 10 runs.
  - `web-v2/src/main.tsx` — lazy-loaded `/plan-migrations` route added below `/recipes`.
  - `web-v2/src/components/Layout.tsx` — sidebar nav entry under Configuration with `Wand2` icon.
  - `CHANGELOG.md` `[Unreleased] / Added` + `web-v2/src/pages/Changelog.tsx` Linear-style entry dated 2026-04-26.
- **Audit-log surface (TWO new event types):**
  - `plan.migration_committed` — one cohort summary entry per commit. Metadata: `from_plan_id`, `to_plan_id`, `effective`, `customer_filter`, `applied_count`, `totals`, `item_errors`, `idempotency_key`, `resource_label`. Resource type: `plan_migration`, resource id: the migration row id.
  - `subscription.plan_changed` — one entry per migrated customer-item. Metadata: `customer_id`, `item_id`, `from_plan_id`, `to_plan_id`, `effective`, `plan_migration_id` (links to the cohort entry above), `resource_label`. Resource type: `subscription`, resource id: the subscription id. Lets CS reps answer "why did my plan change?" tickets without joining tables.
- **Migration number rationale:** picked `0058` from `origin/main` head (per `feedback_migration_numbering`); `0057_*` is the latest committed migration on main as of branching.
- **Approximation called out (not a regression):** the after-preview substitutes `base_fee` lines but does NOT re-rate usage lines against the to-plan's pricing rules. The operator's primary "is this $50 → $100/mo upgrade safe?" question is answered exactly via the base-fee delta. The committed migration row stores this approximation; the next real cycle scan emits the canonical invoice with fully-rated lines, and any drift surfaces as the operator's first post-migration invoice. Documented in `service.go` previewAfter + flagged in the `warnings[]` array when the approximation is in play.
- **Wire-shape regression tests:** all three pass under `go test -short -count=1 -run TestWireShape_PlanMigration ./internal/planmigration/...`.
