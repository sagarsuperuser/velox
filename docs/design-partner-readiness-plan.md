...
#### [T0-20] SMTP bounce/complaint handling (pipeline only) — S — 🟡
...
- **Shipped 2026-04-24** (`6421340`, status marked 🟡 *pipeline only* after 2026-04-24 re-audit): migration 0050 adds `customers.email_status` (NOT NULL, default 'unknown', CHECK in unknown/ok/bounced/complained) + `email_last_bounced_at` + `email_bounce_reason`. Pipeline: SMTP send → `net/smtp` returns 5xx → `Sender.isPermanentSMTPBounce` → `BounceReporter.ReportBounce(tenantID, email, reason)` → blind-index lookup → `customer.MarkEmailBounced` → UPDATE → `customer.email_bounced` webhook event. Customer `Get` + `GetByExternalID` scans widened. SPA: `CustomerDetail` shows a red `Bounced` badge with the reason on hover.
  - **Honest boundary (flagged 2026-04-24):** the pipeline is complete and the UI is ready, but synchronous-SMTP 5xx detection covers only a minority of real-world bounces — most MX providers emit bounces as **async NDRs** that arrive minutes-to-days later at the envelope-from mailbox, not as synchronous `RCPT TO` failures. Full coverage ships with T1-8 SES/SendGrid/Postmark webhook handlers, which call the same `customer.MarkEmailBounced` service method. Treat T0-20 as "ready to receive bounce signal from any source, including future ESP webhooks" rather than "bounces are detected today."
  - Deliberately-deferred (T1-8 work): async NDR parsing, provider webhooks, auto-suppression on subsequent sends, complaint-vs-bounce differentiation.
...
├─ 🟡 T0-20 Bounce handling (pipeline only)  (commit 6421340 — detection is partial; full via T1-8 ESP webhooks)
