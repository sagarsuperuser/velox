...
> **2026-07-07 amendment (ADR-084):** the recipe is a **catalog generator + operator-owned starter plan** — it holds a standing claim only on versioned reference data it fully declares and that never drifts (meters incl. aggregation, rating rules, pricing rules), and **no claim on the plan**, which is operator-owned business config from the moment it exists. On reinstall, a pre-existing plan (by code) is **reused as-is and reported** (a `warnings` entry) — never conform-checked, never refused, never mutated; an operator-edited own plan and a foreign collision get identical treatment. Meters keep a narrow aggregation conformance check (reference data that is billing-consulted); rating rules stay adopt-latest-version (ADR-070). `base_amount_cents` + `base_bill_timing` are now instantiate overrides, so a hybrid (e.g. in-advance $99) plan is seeded in one call with no follow-up edit. This superseded ADR-083's plan conformance gate (which had begun accreting a provenance system to distinguish the two present-plan cases — the wrong-model tell).
>
