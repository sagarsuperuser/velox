...
- [x] React component: `<VeloxCostDashboard />` — typed wrapper at `web-v2/src/components/embeds/VeloxCostDashboard.tsx` accepts `token`, `baseUrl`, `theme`, `accent`, `width`, `height`, `className` props and renders the public iframe. The originally-planned `tenantKey customerId` signature is deferred — that requires a publishable-key auth path on the public endpoint which would weaken the per-customer-token isolation the iframe currently provides. Standalone `@velox/react` npm package + inline (non-iframe) render mode tracked for v1.1.
