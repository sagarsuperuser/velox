...
- [ ] **CSV export carries simulated time as its own column** — with the clock filter on, click **Export**. The CSV has **Simulated Time** and **Test Clock** columns between Date and Details, populated for every simulated row (empty for wall-clock rows), and contains exactly the rows on screen — the export honors the clock filter, it does not silently export everything.
