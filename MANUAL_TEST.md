...
- [ ] **The deletion is auditable:** the clock delete shows up in the audit log (`action=delete`, `resource=test_clock`), so "which clock existed and was removed" stays answerable even though the row itself is gone.
