...
- [ ] **Stripe-side high-precision case (NYC):** invoice an NY customer (10118 / Manhattan) with the `stripe_tax` provider (needs a NY registration in the Stripe test dashboard). Stripe returns `percentage_decimal: "8.875"` in its **document-level** `tax_breakdown`. Check the **line level**: `invoice_line_items.tax_rate = 8.8750` verbatim AND `tax_jurisdiction = US-NY` — seeded from the document-level rate because Stripe leaves the *per-line* breakdown null (only `line_items` is expanded). The **invoice level** `invoices.tax_rate` is the *effective* rate (`tax_amount_cents × 100 / subtotal`), so it reads `8.8800` on a $100 invoice (888¢ → 8.88%) — expected, not precision loss. `subtotal_cents × 8.8750 / 100` round-trips to `tax_amount_cents`. (Pre-fix the line stored the rounded `8.8800` with an empty jurisdiction — regression-guarded by `TestMapResult_DocLevelRateFallback`.)
