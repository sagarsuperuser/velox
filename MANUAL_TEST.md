...
- [ ] **E1 STARTTLS**: `SMTP_TLS=starttls SMTP_PORT=587` + creds. Trigger invoice email → `email_outbox` row pending → dispatched within seconds → recipient receives. *(provider-gated: needs a real STARTTLS SMTP endpoint + creds — Mailpit advertises no TLS; run against the first configured provider.)*
- [ ] **E2 Implicit TLS**: `SMTP_TLS=implicit SMTP_PORT=465`. Same expectation; verifies `tls.Dial` path. *(provider-gated, as E1.)*
...
- [ ] **E4 5xx bounce**: send to `foo@invalid` → `customers.email_status='bounced'`, `email_bounce_reason` carries the SMTP error. *(provider-gated: Mailpit accepts every recipient; needs a rejecting SMTP.)*
- [ ] **E5 Per-provider**: verify SendGrid / Postmark / SES / Mailgun / Resend per `docs/ops/email-setup.md`. *(provider-gated.)*
