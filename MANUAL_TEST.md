...
- [ ] **Failed refunds surface proactively (2026-06-28):** with a credit note at `refund_status=failed` (or `pending`), the **Dashboard** shows a danger alert **"N refunds need attention"** linking to **Credit Notes** filtered to those (`?refund_status=needs_attention`); a dismissible chip reads "Showing refunds needing attention (failed or pending)". The count (`overview.refunds_needing_attention`) excludes test-clock-simulated CNs (`is_simulated=false`). Resolving the refund (Retry refund → succeeded) drops the count and clears the alert.
