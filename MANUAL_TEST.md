...
- [ ] **Threshold invoice on a test clock discloses simulation:** pin a sub with an amount threshold to a test clock, advance until the cap crosses → the threshold invoice's list row shows the **Simulated** chip and its detail page the test-clock banner (is_simulated=true), same as sibling cycle invoices on the clock.
