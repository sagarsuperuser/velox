...
- [ ] **Clock-aware auto-charge tail** — the no-PM banner on a test-clock invoice ends *"…auto-charges this invoice on the next test-clock advance"*; on a wall-clock invoice, *"…on the next billing tick — no operator action needed"*. No second client-rendered copy of the promise below the banner.
- [ ] **Diagnostic detail agrees with the banner on a card-less invoice** — the run row reads "Payment recovery · No retries yet" (or "N retries", never "attempt 0"), and the date row is **"Next reminder"** with muted "Won't charge until a card is attached." Decline (has-card) dunning keeps "Next retry".
- [ ] **No dead Collect button** — with no PM on file the disabled "Collect Payment" primary is absent; it returns once a card is attached.
- [ ] **dunning_exhausted** — let a no-PM run exhaust (final action fires, run `state=escalated`) → banner becomes *"Payment recovery has ended without collecting…"* (code `payment.dunning_exhausted`, label "Payment recovery ended"), still offering Resend setup link (with email) / Open customer page; Diagnostic badge reads **ended** with no future-step row. Settling the invoice clears it.
...
- [ ] **Proactive no-card warning (customer page)** — a customer with an active auto-charging subscription and zero cards shows the amber "No payment method on file … bills automatically" warning in the Payment methods card; a card-less customer with **no** active subs keeps the neutral empty state.
- [ ] **Proactive no-card banner (subscription page)** — that customer's subscription page shows the amber **"No card on file"** banner with an "Add payment method" button → customer page; attaching a card or pausing collection removes it.
- [ ] **Create-subscription nudge** — creating a start-now subscription for a card-less customer succeeds AND raises a "No payment method on file" warning toast; a customer with a card gets no toast.
