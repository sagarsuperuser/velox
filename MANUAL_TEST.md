...
- [ ] **Downgrade/removal clawback is atomic + recoverable (ADR-057, migration 0121)**: a downgrade / qty-decrease / RemoveItem on a PAID source issues a tax-reversing **clawback credit note**, created as a **DRAFT inside the item-change tx** (`issue_pending`) and issued post-commit. **Atomicity:** `REVOKE INSERT ON credit_notes FROM velox_app` (the clawback writes `credit_notes`, NOT `customer_credit_ledger`), RemoveItem a paid-source item → the item is **still present** (the draft-create failure rolls the delete back), not deleted-with-no-credit; `GRANT` back, retry → succeeds. **Recovery (Issue-never-ran):** if instead the *post-commit* `Issue()` never runs (kill the process right after commit), the draft stays `status='draft' AND issue_pending=true` and `RetryPendingClawbackIssue` (scheduler tick) re-issues it on the next tick. **Partial-issue window now closed (ADR-061, 2026-06-25):** `Issue()` is atomic — the `draft→issued` flip and the internal grant/`amount_due` effect commit on one tx, so there is no `status='issued'` row with an un-applied internal effect to reconcile by hand; a post-flip *external*-leg failure (refund / tax reversal) self-heals via `RetryRefund` / `RetryPendingCreditNoteTaxReversal`. (Pre-fix the clawback was created+issued post-commit fire-and-forget: any failure removed the item but left the customer un-credited with only a "reconcile manually" log.)
...
- [ ] **Issuing a credit note is atomic — a failed internal effect leaves no orphan (2026-06-25, ADR-061):** on a **paid** invoice, create a credit-type credit note (credit-to-balance), then `REVOKE INSERT ON customer_credit_ledger FROM velox_app` and Issue it → the action **fails** and `SELECT status FROM credit_notes WHERE id='<cn>'` is still **`draft`** (the `draft→issued` flip rolled back with the failed grant — no "issued" credit the customer never received); `GRANT INSERT ON customer_credit_ledger TO velox_app` and re-issue → succeeds, status `issued`, balance credited once. (Pre-fix the flip committed in its own tx, so the grant failure left an `issued` credit note with no balance credit and no auto-recovery.)
...
- [ ] **A failed tax reversal on void self-heals via the scheduler sweep (2026-06-24, migration 0122):** on a `stripe_tax` invoice with a committed `tax_transaction_id`, make the upstream `ReverseTax` call fail (point `stripe listen` away / break Stripe creds), then Void it. The invoice **voids** locally, an **ERROR** log fires (`tax reversal failed on invoice status change — … recovery sweep will retry`), and `SELECT tax_reversed_at FROM invoices WHERE id='<inv>'` is **NULL**. Restore Stripe → on the next scheduler tick the **reversal recovery sweep** files the reversal at Stripe **exactly once** (idempotent via the `inv_taxrev_<id>` reference) and stamps `tax_reversed_at` non-NULL; the row then falls out of the sweep (no repeat reversals). A fully-credit-noted void (nothing left to reverse) is also stamped, so it never loops. (Clock-pinned/test invoices are excluded — the sweep is wall-clock/livemode only.)
- [ ] **A failed credit-note tax reversal self-heals via its own sweep (2026-06-25, ADR-061, migration 0123):** on a **finalized/paid** `stripe_tax` invoice with a committed `tax_transaction_id`, make `ReverseTax` fail (break Stripe creds), then **issue a credit note** against it. The credit note **issues** (its internal money effect committed atomically), an **ERROR** log fires (`tax reversal failed; marked pending for sweep recovery`), and `SELECT tax_reversal_pending FROM credit_notes WHERE id='<cn>'` is **true**. Restore Stripe → on the next scheduler tick `RetryPendingCreditNoteTaxReversal` files the reversal **exactly once** (idempotent via the per-CN `velox_tax_rev_<cn_id>` key), stamps `credit_notes.tax_transaction_id`, and clears the marker → the row falls out of the sweep. This is a **separate** sweep from the void-path one above: #310 scans only voided/uncollectible invoices, so it never sees a reversal stamped on this finalized/paid invoice. (Pre-fix this was fire-and-forget — a failed reversal silently over-remitted with no retry. Clock-pinned/test credit notes are excluded — wall-clock/livemode only.)
- [ ] **Recovery sweeps are observable per-reconciler (2026-06-25):** while the stuck reversal above is pending, `GET /metrics` shows `velox_reconciler_sweeps_total{reconciler="tax_reversal",outcome="run"}` (or `="cn_tax_reversal"`) **incrementing every scheduler tick** (liveness) and `outcome="error"` climbing while it fails; once Stripe is restored, `outcome="advanced"` ticks up and `error` goes flat. All six sweeps emit it (`payment_unknown`, `tax_retry`, `tax_commit`, `tax_reversal`, `clawback_issue`, `cn_tax_reversal`); pre-2026-06-25 only `velox_auto_charge_retries_total` existed, so a stuck backlog was invisible.
