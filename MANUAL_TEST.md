...
- [ ] **A cross-site POST cannot log you out.** From a page on a *different site* (`127.0.0.1` vs `localhost` — SameSite compares hostnames, ports are irrelevant), submit a form to `POST http://localhost:5173/v1/auth/logout`. SameSite=Lax withholds the cookie, so the server sees no session: it must answer `204` with **no `Set-Cookie` header**, you stay signed in, and the session row keeps `revoked_at IS NULL`. (Before the 2026-07-16 fix the reply carried `Set-Cookie: velox_session=; Max-Age=0` and the browser honoured it — any site could force-logout an operator while the session stayed live server-side.) A plain **link** from that same cross-site page to the dashboard *does* keep you signed in — that is Lax rather than Strict. *(automated: `TestLogout_NoCookie_DoesNotClearTheCookie`, `TestLogout_Success_StillClearsTheCookie`)*
