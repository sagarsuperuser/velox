...
- [ ] 3 line items $33.33+$33.33+$33.34 at 7.25%: `SUM(invoice_line_items.tax_amount_cents) = invoices.tax_amount_cents = 725` exactly, AND the residual lands by largest remainder, not on the last line (ADR-046). Per-line `tax_amount_cents`: the two $33.33 lines and the $33.34 line each compute to 242¢ naively (sum 726¢), so one cent is clawed back from the line with the smallest fractional remainder — a $33.33 line → `242 / 241 / 242`. The $33.34 line must NOT be the one docked: no line with a larger `amount_cents` may carry a smaller `tax_amount_cents` than a smaller line.
