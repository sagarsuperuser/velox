...
- [x] **E1 STARTTLS**: `SMTP_TLS=starttls SMTP_PORT=587` + creds. Trigger invoice email → `email_outbox` row pending → dispatched within seconds. *(manual 2026-07-19: verified against BOTH the Mailtrap sandbox (587 STARTTLS, dispatched attempts=1) AND real production Postmark (smtp.postmarkapp.com:587 — connect→STARTTLS→AUTH→MAIL/RCPT/DATA→250, dispatched attempts=1, no error; Postmark accepting the send also confirms the verified sender signature). Creds live in gitignored .env Option 4.)*
...
- [~] **E5 Per-provider**: verify SendGrid / Postmark / SES / Mailgun / Resend per `docs/ops/email-setup.md`. *(manual 2026-07-19: **Postmark** verified end-to-end — real STARTTLS send accepted (250), Server-Token-as-both-username-and-password auth, verified sender signature. SendGrid/SES/Mailgun/Resend remain provider-gated — no creds. .env Option 4 documents the Postmark shape.)*
