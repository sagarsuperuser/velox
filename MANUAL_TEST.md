...
- [~] **E1 STARTTLS**: `SMTP_TLS=starttls SMTP_PORT=587` + creds. Trigger invoice email → `email_outbox` row pending → dispatched within seconds. *(manual 2026-07-19 against the Mailtrap sandbox creds in .env: strict-STARTTLS connect→upgrade→AUTH→250, row dispatched attempts=1, no error. "Recipient receives" rendering re-verified per provider at real-provider setup.)*
