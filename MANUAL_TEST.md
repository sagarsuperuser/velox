...
- [ ] **Activity vs Notifications lanes don't interleave clocks** — on a clock-pinned invoice that has been finalized + emailed, the invoice detail shows two cards: **Activity** (billing lifecycle, simulated dates, per-row simulated badge) and **Notifications** (customer emails, real wall-clock send times, no simulated badge). The email "sent" rows are NOT mixed into the Activity list (which would sort a real-time row before the simulated event that triggered it).
