...
- [ ] **`request_id` is server-minted, never client-chosen**: make any mutating call with a forged header — `curl -X POST … -H 'X-Request-Id: forged-correlation-id'`. The resulting audit row's expanded **Request ID** shows a server id (prefixed `req_`), NOT `forged-correlation-id`, and the `Velox-Request-Id` response header carries that same server value. chi's stock `middleware.RequestID` honoured the inbound header verbatim, which let a caller choose the correlation id recorded on their own audit rows — correlation evidence an adversary can write is not evidence. *(automated: `TestRequestID_IgnoresClientSuppliedHeader`)*
