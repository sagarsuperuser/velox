...
- [ ] **No stranded sub after clock delete (2026-06-13):** after the delete above, create a NEW subscription for one of the (now detached) customers. It is a normal **wall-clock** sub — `test_clock_id` is NULL, it appears for the wall-clock scheduler, and it bills on the next tick / advances its period. Pre-fix it inherited the deleted clock and never billed (excluded from both the cron and the catchup path).
...
- [ ] **Quantity increases as TWO lines (2026-06-12):** raise a $10.00/seat item from 1 → 3 seats at exactly half period. The proration invoice shows *"Unused time on 1 × Seat (after &lt;date&gt;)"* **−$5.00** (qty 1, unit −$5.00) and *"Remaining time on 3 × Seat (after &lt;date&gt;)"* **+$15.00** (qty 3, unit **$5.00** — the true prorated per-seat rate), net **$10.00**. On BOTH lines qty × unit price = amount exactly. Pre-fix: one line with qty 3, unit $3.33, amount $10.00 — 3 × $3.33 ≠ $10.00 on the face of the invoice.
- [ ] **The plan-change proration invoice carries the "Simulated" badge on a test clock (2026-06-09):** on a clock-pinned in_advance sub, advance the clock and upgrade the plan immediately. The resulting proration invoice — detail header (next to the status pill) and its Invoices-list row — shows the amber **Simulated** badge, exactly like the sibling cycle invoice on the same sub (`invoices.is_simulated = true` in DB). Pre-fix only the engine's cycle invoices were flagged; the handler-built proration invoice persisted `is_simulated=false` (no badge) despite its issued/created dates being on simulated time.
...
- [ ] Operator clicks **Collect payment** on a finalized invoice → "Collected payment on INV-NNN" (amber/medium severity, `action='collect'`), NOT "Created INV-NNN". Operator clicks **Send** → "Emailed INV-NNN" (`action='send'`; metadata carries the invoice + customer ids, NOT the recipient address — PII stays out of the append-only log; the email outbox is the delivery record). Operator **Refund** → "Refunded INV-NNN" (red/high severity). Operator rotates the hosted-invoice link → "Rotated hosted-invoice link for INV-NNN". None of these render as a green "Created" row.
