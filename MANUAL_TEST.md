...
- [ ] Mode-scoped pages reflect the new mode after toggle (no stale rows): Customers, Invoices, Subscriptions, API Keys, Audit Log, Usage Events, Credits, Credit Notes, Dunning, Coupons, Pricing (plans / meters / rating rules), Webhooks (endpoints + event stream), Dashboard (MRR / active customers / recent invoices).
- [ ] Mode-agnostic pages stay the same on toggle (no stale-data risk by definition): Settings (General / Invoicing / Tax / Payments), Recipes, Onboarding. The keyed remount still refetches but content is identical.
