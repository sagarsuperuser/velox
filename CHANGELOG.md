...
- **Settings tax-rate example completes the ADR-043 migration (2026-05-31).** The inline tax-example preview (`web-v2/src/pages/Settings.tsx`) still referenced the dropped `form.tax_rate_bp` in three spots after ADR-043 renamed it to `tax_rate` (decimal percent). The dev server hid this (vite strips types without checking); the production `tsc -b` build flagged it. Converted to `tax_rate` (bp = `tax_rate * 100` in the inclusive/exclusive example math).
