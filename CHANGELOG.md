...
- **Simulated/test-clock markers de-duplicated on detail pages (2026-06-03).** A detail page already carries the simulation context once — the page-level "Currently at &lt;sim time&gt;" banner plus a single entity marker in the header. Repeating a "Test clock" badge on every embedded-list row underneath was noise: on Customer Detail, every subscription row and every recent-invoice row shares the one customer's clock, so the badge said nothing the banner didn't. Removed the per-row badges from Customer Detail's embedded **subscriptions** and **recent-invoices** lists, and dropped the redundant second header chip on Invoice Detail (the invoice header showed both a "Test clock" chip next to the number *and* a "Simulated" chip next to the status pill, on top of the banner — now just the authoritative "Simulated" chip, which also covers manual invoices). **Unchanged:** the page banners, the single header entity marker on Customer/Subscription/Invoice detail, the activity-timeline event chips (informative — they distinguish simulated billing rows from wall-clock email/Stripe rows on a mixed timeline), and **all** badges on the global list pages (Dashboard, Customers, Subscriptions, Invoices, Dunning) where rows belong to different entities and a per-row marker is the only signal. Rule of thumb: a per-row badge earns its place only where rows can differ; on an entity-detail page where every row inherits the parent's clock, the page-level banner is the single source. Frontend-only.
