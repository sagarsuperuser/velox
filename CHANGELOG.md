...
- **Calendar-billing subs auto-realign at cycle close (Velox-specific divergence from preserve-anchor industry norm).** Adopts a position partway between Stripe-flexible/Lago/Chargebee (always preserve day-of-month) and Stripe-legacy/Recurly (always re-anchor on interval change): **respect the operator's configured `billing_time`**. Calendar+monthly cycles snap to first-of-next-month at every cycle close, even when the prior period started mid-month (e.g. inherited from a yearly→monthly plan swap). Anniversary subs preserve day-of-month as before (unchanged).
  ...
  - The `Reset billing cycle` operator action shipped earlier in this PR-set remains useful for operators who want to re-anchor immediately rather than wait one cycle.
- **"Reset billing cycle" — financial-math redesign + int64-overflow fix (2026-05-21).** The initial Reset shape (compute new period at anchor immediately) leaked revenue for in_advance plans: it refunded the unused portion of the original prepayment but never billed the new period, giving customers free service for the overlap. Plus the proration math used nanosecond arithmetic which overflowed int64 for plans > ~$36 base — silently returned 0 credit on real-world plan sizes.
  - **Cross-platform research** (Stripe + Lago + Chargebee + Recurly, quoted source lines) found three-platform convergence on the correct shape: refund unused old portion + bill new period. Stripe does both on a combined invoice; Chargebee via Term Change; Lago via terminate+recreate. Recurly is the outlier (no proration on Postpone — operator-explicit decision).
  - **New Velox model** (simpler than Stripe's combined-invoice approach, mathematically equivalent): truncate `current_period_end` to anchor; issue credit for in_advance unused portion at reset time; let the natural cycle close at anchor fire the new in_advance invoice (existing `billOnePeriod` path). For in_arrears, the truncated portion bills naturally at anchor close via `emitBaseSegmentLine` (existing path). No new engine methods required.
  - **Customer math closes** for the running example ($59 plan, paid Jun 24 → Jul 24, reset to Jul 1): $59 (original) − $45.23 (credit) + $59 (new in_advance at Jul 1 cycle close) = $72.77 over 38 days ≈ plan rate.
  - **int64-overflow fix**: both `BillOnCancel` and `BillOnCycleReset` swapped nanosecond math (`baseFee × unusedNanos / periodNanos`) for day-based math (`baseFee × unusedDays / periodDays`). Latent in cancel proration too — any in_advance plan with base > ~$36 silently issued 0 cancel credit pre-fix.
  - **TZ-correctness in the "Start of next month" preset**: dialog computed the date via `new Date(y, m, 1).toISOString().slice(0, 10)` (browser-local Date round-trip through UTC), which shifted the date back a day when browser TZ is ahead of UTC (IST +5:30 → "July 1 IST" round-tripped to "2029-06-30"). Anchor landed at Jun 30 → anniversary subs rolled to Jul 30 instead of Aug 1. Fixed by computing YYYY-MM-DD directly from the tenant-TZ year/month.
  - **Test-clock awareness in dialog presets**: pre-fix the three preset dates and DatePicker `minDate` all used wall-clock `new Date()`, ignoring the sub's `test_clock_id`. Operator on a 2029-simulated sub saw "Today = May 21 2026" while server's anchor validation ran against simulated clock. Now uses `testClock.frozen_time` for clock-pinned subs.
  - **Audit/event payload renamed**: `new_period_start`/`new_period_end` → `truncated_period_end` (new period boundaries are implicit — naturally computed at cycle close by the existing `domain.NextBillingPeriodEnd`). Renderer reads either key for backwards-compat on pre-redesign rows.
  - MANUAL_TEST FLOW SUB-RESET rewritten to match the truncate model + adds explicit financial-math regression checks (plans > $36 base, TZ-correct preset, test-clock simulated time). FLOW SUB-REALIGN added for the cycle-close auto-realign behavior shipped earlier. FLOW B17 (cancel proration) gets the same int64-overflow regression check.
- **Operator-driven "Reset billing cycle" — Stripe + Chargebee + Recurly parity for re-anchoring after plan-interval-change drift.** Closes the escape-hatch gap that the prior commit (renews-on annotation) only papered over with copy. Cross-platform research found three of four reference platforms surface this as a dashboard action: Stripe "Bill today instead" (overflow menu), Chargebee "Change Next Billing Date", Recurly "Update billing date" (subscription edit). Lago is API-only. Velox now matches the three-of-four convergence.
  - **New endpoint** `POST /v1/subscriptions/{id}/reset-billing-cycle` with body `{ "anchor_at": "<RFC3339>" }`. Validation: sub status=active, anchor_at in `(now, current_period_end)` — future-but-before-natural-close. Anything after period end → wait for natural cycle close; past anchors → reject. Returns the updated sub + `proration_credit_cents`.
  - **Service-layer impl** `subscription.Service.ResetBillingCycle` reuses `firstPeriodForActivate` for the new-period anchor (calendar-snap for calendar billing, anniversary +interval otherwise) — same helper that runs at sub create / Activate / EndTrialEarly so the math is consistent across lifecycle entry points. Calls the biller's new `BillOnCycleReset` for in_advance proration; best-effort (logged on failure, doesn't abort the period update — matches cancel-proration's stance).
  - **Engine `BillOnCycleReset`** mirrors `BillOnCancel`: computes `(periodEnd − anchorAt) / (periodEnd − periodStart) × baseFee` for each in_advance item, grants a credit via `creditGranter.Grant`. Same paid-check gate as cancel proration — unpaid source invoices skip the credit (Chargebee Adjustment / Stripe `proration_behavior=none` parity).
  - **Audit + webhook** `subscription.billing_cycle_reset` carries old + new period boundaries + proration_credit_cents. Activity timeline renders "Billing cycle reset — Proration credit $X.YY issued · Renews <date>".
  - **SPA dialog** with three preset options matching common operator intents:
    - **Start of next month** (recommended) — calendar-fix for the user's exact case: post-yearly→monthly anchor drift. Default selection.
    - **Today** — Stripe "Bill today instead" parity. Re-anchor immediately.
    - **Custom date** — DatePicker with min=today, max=current_period_end. Honors the server-side gate.
  - **Tests**: `TestResetBillingCycle` covers calendar-snap math, anniversary roll-by-interval math, four guard conditions (past anchor, anchor at/after period end, non-active sub, biller-failure-doesn't-abort).
- **Subscription billing-cycle card surfaces "Renews on <date>" + alignment tooltip (Stripe + Lago + Chargebee + Recurly research, 2026-05-20).** Cross-platform research found three converging patterns: (1) show the *computed* next-renewal date as a concrete primary label (Recurly "Renews On", Chargebee "Next Billing Date", Stripe's `current_period_end`); (2) keep the configured `billing_time` policy separate from the computed dates (Lago's `billing_time` vs `subscription_at` shape); (3) plan-interval changes preserve the historical day-of-month anchor (Stripe `billing_mode=flexible` / Lago / Chargebee — three-platform convergence; only Stripe legacy mode + Recurly auto-re-anchor). Velox already matches the preserve-anchor behavior; the UX was misleading because the page showed `Calendar` + period like `May 20 → Jun 20` and operators reasonably assumed bug. The fix in this PR is UX-only — behavior is unchanged.
  ...
  - Deferred to a follow-up: an explicit **"Reset billing cycle"** operator action (Stripe "Bill today instead" / Chargebee "Change Next Billing date" parity). Requires proration design; current PR is dashboard-only and behavior-preserving.
